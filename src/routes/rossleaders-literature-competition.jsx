...
const RossLiteratureComp = () => {
  ...
  useEffect(() => {
    ...
    .then (data => { 
      data[data.length-1].forEach(info => {
        if (info.competitionLink == '/rossleaders-literature-competition') {
  ...
  useEffect(() => {
    fetch('/data/rossLiteratureCards.json')
    .then(res => res.json())
...
export default RossLiteratureComp