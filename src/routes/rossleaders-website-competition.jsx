...
const RossWebsiteComp = () => {
  const [rossWebsiteCards, setRossWebsiteCards] = useState([]);
  ...
  useEffect(() => {
    ...
    .then (data => { 
      data[data.length-1].forEach(info => {
        if (info.competitionLink == '/rossleaders-website-competition') {
  ...
  useEffect(() => {
    fetch('/data/rossWebsiteCards.json')
    .then(res => res.json())
    .then(data => setRossWebsiteCards(data))
  ...
  return (
    <div className="page compete-page__challenge">
      ...
      <div className="competition__cards">
        {rossWebsiteCards.map( meducationCard => (
          <CompetitionCard 
            key={uuidv4()} 
...
export default RossWebsiteComp